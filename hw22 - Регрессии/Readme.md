...
В данном задании лучшие результаты показала модель **CatBooster** с показателем MAPE 3.35%, что, я считаю, является отличным показателем для такого малого набора данных. Так же стоит отметить простоту ее использования - достаточно указать колонки с категориальными данными и подать подобный сет в его исходном виде, без ручной кодировки признаков. Всю обработку признаков модель берет на себя, упрощая прототипирование.
